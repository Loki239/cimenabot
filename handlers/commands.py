...
async def cmd_clear_cache(message: Message):
    ...
    user_id = message.from_user.id
    logging.info("Очистка всего кэша пользователем %s", user_id)
...
async def cmd_clear_posters(message: Message):
    ...
    user_id = message.from_user.id
    logging.info("Очистка кэша постеров пользователем %s", user_id)
    
    await clear_posters()
    await message.answer("✅ Кэш постеров очищен")
...
async def cmd_clear_movie_data(message: Message):
    ...
    user_id = message.from_user.id
    logging.info("Очистка кэша данных фильмов пользователем %s", user_id)
    
    await clear_movie_data()
    await message.answer("✅ Кэш данных фильмов очищен")
...
async def cmd_clear_rutube(message: Message):
    ...
    user_id = message.from_user.id
    logging.info("Очистка кэша ссылок Rutube пользователем %s", user_id)
    
    await clear_rutube()
    await message.answer("✅ Кэш ссылок Rutube очищен")
